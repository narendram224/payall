          ...
          <View className="flex-row gap-2">
            {NETWORKS.map((n) => (
              <Pressable
                ...
                className={`flex-1 items-center rounded-xl border py-3 ${network?.id === n.id ? 'border-indigo-500 bg-indigo-50' : 'border-border bg-card'}`}>
                <Text className="text-xl">{n.emoji}</Text>
                <Text
                  className={`mt-1 text-xs font-bold ${network?.id === n.id ? 'text-indigo-600' : 'text-foreground'}`}>
                  {n.name}
                </Text>
              </Pressable>
            ))}
        ...
        <Animated.View
          ...
          {QUICK.map((a) => (
            <Pressable
              ...
              className={`rounded-full border px-4 py-2 ${amount === String(a) ? 'border-indigo-500 bg-indigo-50' : 'border-border bg-card'}`}>
              <Text
                className={`text-sm font-bold ${amount === String(a) ? 'text-indigo-600' : 'text-foreground'}`}>
