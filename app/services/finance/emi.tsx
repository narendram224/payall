...
export default function EMICalculator() {
  ...
  const principalPct =
    totalAmount > 0 ? Math.round((parseFloat(principal) / totalAmount) * 100) : 0;
    ...
    <View className="flex-1 bg-[#1c1c1c]">
      ...
      <ScrollView
        ...
        <Animated.View
          ...
          className="mb-5 rounded-2xl border border-indigo-800 bg-[#1e1b4b] p-4">
          <Text className="mb-2 text-base font-bold text-white">🧮 EMI Calculator</Text>
          <Text className="mb-3 text-xs text-indigo-300">
            Understand your loan repayment before you commit
          </Text>
          {[
            'Compare different loan amounts',
            'See exact monthly outflow',
          ...
          <View className="flex-row flex-wrap gap-2">
            {TENURES.map((t) => (
              <Pressable
                key={t}
                onPress={() => setTenure(t)}
                className={`rounded-full border px-4 py-2 ${tenure === t ? 'border-primary bg-primary/10' : 'border-border bg-card'}`}>
                <Text
                  className={`text-sm font-bold ${tenure === t ? 'text-primary' : 'text-foreground'}`}>
                  {t} mo
                </Text>
              </Pressable>
            ))}
        ...
        {/* Result Card */}
        <Animated.View
          entering={reducedMotion ? undefined : FadeInDown.delay(240).duration(350)}
          ...
          <View className="bg-card px-5 py-4">
            ...
            <View className="h-3 overflow-hidden rounded-full bg-orange-100">
              <View
