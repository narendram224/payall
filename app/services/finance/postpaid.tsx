...
export default function PostpaidBill() {
  ...
  const handleProceed = () => {
    if (!selected) { toast.error('Select operator'); return; }
  ...
  const handleConfirm = async () => {
    try {
      setLoading(true);
      const res = await rechargeService.recharge({ number: mobile, provider_id: selected!.id, amount, client_id: `PP${Date.now()}` });
      if (res.status_id === 1) { setResult(res); setShowSummary(false); }
      else toast.error(res.message ?? 'Payment failed');
    } catch (e: any) { toast.error(e?.response?.data?.message ?? 'Payment failed'); }
    finally { setLoading(false); }
  };

  if (result) return <SuccessScreen orderId={result.order_id} amount={result.amount} message="Postpaid bill paid!" onDone={() => { setResult(null); setSelected(null); setMobile(''); setAmount(''); setShowSummary(false); router.back(); }} />;
        ...
        {!showSummary ? (
          <>
            <Animated.View entering={reducedMotion ? undefined : FadeInDown.delay(0).duration(300)} className="mb-4">
              ...
              <OperatorSelector operators={providers} selectedOperator={selected} onSelectOperator={setSelected} />
            </Animated.View>
            ...
            <Animated.View entering={reducedMotion ? undefined : FadeInDown.delay(160).duration(300)} className="mb-5">
              <AmountInput value={amount} onChangeText={setAmount} quickAmounts={[199, 299, 399, 499, 699, 999]} />
            </Animated.View>
          ...
          <Animated.View entering={reducedMotion ? undefined : FadeInDown.duration(300)} className="space-y-4">
            <View className="rounded-2xl border border-border bg-card p-4">
              <TransactionSummary operatorName={selected?.provider_name ?? ''} number={mobile} amount={amount} />
            </View>
            <View className="flex-row gap-3 mt-2">
              <Pressable onPress={() => setShowSummary(false)} className="flex-1 items-center rounded-2xl border border-border py-4">
