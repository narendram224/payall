...
const GiftCard = () => {
  ...
  const giftCards = [
    { id: '1', name: 'Google Play Gift Card', brand: 'Google Play', icon: '', minAmount: 100, maxAmount: 10000 },
    { id: '2', name: 'Amazon Gift Card', brand: 'Amazon', icon: '', minAmount: 100, maxAmount: 10000 },
    { id: '3', name: 'Netflix Gift Card', brand: 'Netflix', icon: '', minAmount: 500, maxAmount: 5000 },
  ];

  const presetAmounts = [100, 200, 500, 1000, 2000, 5000];

  const handleProceed = () => {
    if (!selectedCard) { Alert.alert('Error', 'Please select a gift card'); return; }
    if (!recipientName || recipientName.length < 3) { Alert.alert('Error', 'Please enter recipient name'); return; }
    if (!amount || parseFloat(amount) < selectedCard.minAmount) { Alert.alert('Error', `Minimum amount is ₹${selectedCard.minAmount}`); return; }
    setShowSummary(true);
  };
  ...
  const handlePurchase = async () => {
    try {
      setLoading(true);
      setTransactionResult({ orderId: 'GC' + Date.now(), amount, status: 'success' });
      setShowSuccess(true);
    } catch {
  ...
  return (
    <ScrollView className="flex-1 bg-background" contentContainerStyle={{ paddingHorizontal: 16, paddingTop: 12, paddingBottom: 24 }}>
      <View className="mb-2">
        <Text className="text-xs font-medium text-muted-foreground tracking-wider uppercase">GIFT CARD</Text>
      </View>
        ...
        <View className="flex-row flex-wrap gap-2 mt-2">
          {presetAmounts.map((preset) => (
            <Pressable
              ...
              className={cn(
                'min-w-[70px] items-center justify-center rounded-lg border-2 px-3 py-2',
