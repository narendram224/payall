...
export default function CarInsurance() {
  ...
  const mobileError =
    mobile && !/^[6-9][0-9]{9}$/.test(mobile) ? 'Enter valid 10-digit mobile' : '';
  const vehicleError =
    vehicleNumber && !/^[A-Z]{2}[0-9]{2}[A-Z]{1,2}[0-9]{4}$/.test(vehicleNumber.toUpperCase())
      ? 'Invalid format (e.g. MH02CD5678)'
      : '';

  const handleSubmit = async () => {
    if (!vehicleNumber || !vehicleModel || !year || !fuelType || !seats || !ownerName || !mobile) {
      toast.error('Fill all fields');
      return;
    }
    if (mobileError || vehicleError) {
    ...
    <View className="flex-1 bg-background">
      ...
      <ScrollView
        ...
        {[
          {
            ...
            val: vehicleNumber,
            set: (t: string) => setVehicleNumber(t.toUpperCase()),
            ph: 'e.g. MH02CD5678',
            err: vehicleError,
            cap: 'characters' as any,
            max: 10,
          },
          {
            label: 'Vehicle Model',
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false} className="-mx-4 px-4">
            {YEARS.map((y) => (
              <Pressable
                key={y}
                onPress={() => setYear(y)}
                className={`mr-2 rounded-full border px-4 py-2 ${year === y ? 'border-blue-500 bg-blue-50' : 'border-border bg-card'}`}>
                <Text
                  className={`text-sm font-bold ${year === y ? 'text-blue-600' : 'text-foreground'}`}>
                  {y}
                </Text>
              </Pressable>
            ))}
          ...
          <View className="flex-row flex-wrap gap-2">
            {FUEL_TYPES.map((f) => (
              <Pressable
                ...
                className={`rounded-full border px-4 py-2 ${fuelType === f ? 'border-blue-500 bg-blue-50' : 'border-border bg-card'}`}>
                <Text
                  className={`text-sm font-bold ${fuelType === f ? 'text-blue-600' : 'text-foreground'}`}>
                  {f}
                </Text>
              </Pressable>
            ))}
        ...
        <Animated.View
          ...
          className="mb-4">
          <Text className="mb-2 text-sm font-bold text-foreground">
            Seating Capacity <Text className="text-red-500">*</Text>
          </Text>
          <View className="flex-row gap-3">
            {SEATS.map((s) => (
              <Pressable
                key={s}
                onPress={() => setSeats(s)}
                className={`flex-1 items-center rounded-xl border py-3 ${seats === s ? 'border-blue-500 bg-blue-50' : 'border-border bg-card'}`}>
                <Text
                  className={`text-sm font-bold ${seats === s ? 'text-blue-600' : 'text-foreground'}`}>
                  {s} Seats
                </Text>
              </Pressable>
            ))}
        ...
        <Animated.View
          entering={reducedMotion ? undefined : FadeInDown.delay(480).duration(300)}
          className="mb-6">
          <Text className="mb-1.5 text-sm font-bold text-foreground">
          ...
          <Pressable onPress={handleSubmit} disabled={loading}>
            <LinearGradient
              ...
              {loading ? (
                ...
                <Text className="text-base font-bold text-white">Get Car Insurance Quote →</Text>
              )}
            </LinearGradient>
          </Pressable>
        </Animated.View>
