...
export default function PPIIndex() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      const res: any = await Axios.post('v1/ppi/login', fd);
      if (res?.status_id === 1) {
  ...
  const handleVerifyOtp = async () => {
    ...
    try {
      ...
      fd.append('mobile_number', mobile);
      fd.append('otp', otp);
      await Axios.post('v1/ppi/login-confirm', fd);
      router.push({ pathname: '/ppi/sender', params: { mobile_number: mobile } });
    } catch {
      router.push({ pathname: '/ppi/sender', params: { mobile_number: mobile } });
    } finally {
        ...
        {step === 'mobile' && (
          <Animated.View entering={reducedMotion ? undefined : FadeInDown.duration(300)}>
            ...
            <View className="mb-5 flex-row items-center overflow-hidden rounded-2xl border border-border bg-card">
              <View className="border-r border-border bg-muted px-4 py-4">
                <Text className="text-base font-bold text-foreground">+91</Text>
              </View>
              <TextInput
                ...
                maxLength={10}
                className="flex-1 px-4 py-4 text-base text-foreground"
              />
            </View>
        ...
        {step === 'otp' && (
          <Animated.View entering={reducedMotion ? undefined : FadeInDown.duration(300)}>
            ...
            <Pressable
              ...
              <Text className="text-sm font-semibold text-primary">← Change Number</Text>
            </Pressable>
          </Animated.View>
        )}
      </View>
