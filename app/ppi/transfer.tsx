...
export default function PPITransfer() {
  ...
  const params = useLocalSearchParams<{ mobile_number: string; beneficiary_id: string; beneficiary_name: string; account_number: string }>();
  ...
  const handleGenerateOtp = async () => {
    ...
    try {
      ...
      await apiClient.post('v2/dmt/generate_otp', fd);
      setOtpSent(true);
  ...
  const handleTransfer = async () => {
    ...
    try {
      ...
      const res: any = await apiClient.post('v2/dmt/transfer', fd);
      if (res?.status_id === 1) {
        setResult(res);
      } else {
  ...
  if (result) {
    return <SuccessScreen orderId={result.order_id ?? result.txnId ?? 'PPI' + Date.now()} amount={amount} message={`₹${amount} transferred via PPI`} onDone={() => { router.replace('/ppi/beneficiaries'); }} />;
  }

  return (
    <View className="flex-1 bg-background">
      <LinearGradient colors={['#8b5cf6', '#6366f1']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} className="px-4 pb-6 pt-4">
      ...
      <ScrollView className="flex-1 px-4 pt-5" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 40 }}>
        {/* Beneficiary Card */}
        <Animated.View entering={reducedMotion ? undefined : FadeInDown.delay(0).duration(300)} className="mb-5 rounded-2xl border border-border bg-card p-4">
          <View className="flex-row items-center gap-3">
            <View className="h-11 w-11 items-center justify-center rounded-full bg-purple-100">
              <Text className="text-base font-bold text-purple-600">{params.beneficiary_name?.charAt(0)?.toUpperCase()}</Text>
            </View>
            <View>
              <Text className="text-sm font-bold text-foreground">{params.beneficiary_name}</Text>
              <Text className="text-xs text-muted-foreground">••••{params.account_number?.slice(-4)}</Text>
            </View>
          </View>
        </Animated.View>
        ...
        <Animated.View entering={reducedMotion ? undefined : FadeInDown.delay(80).duration(300)} className="mb-5">
          ...
          <View className="flex-row gap-2">
            {MODES.map(m => (
              <Pressable key={m} onPress={() => setMode(m)} className={`flex-1 items-center rounded-xl border py-3 ${mode === m ? 'border-purple-500 bg-purple-50' : 'border-border bg-card'}`}>
                <Text className={`text-sm font-bold ${mode === m ? 'text-purple-600' : 'text-foreground'}`}>{m}</Text>
              </Pressable>
            ))}
          </View>
        </Animated.View>
        ...
        <Animated.View entering={reducedMotion ? undefined : FadeInDown.delay(240).duration(300)} className="mb-5 flex-row flex-wrap gap-2">
          {QUICK.map(a => (
            <Pressable key={a} onPress={() => setAmount(String(a))} className={`rounded-full border px-4 py-2 ${amount === String(a) ? 'border-purple-500 bg-purple-50' : 'border-border bg-card'}`}>
              <Text className={`text-sm font-bold ${amount === String(a) ? 'text-purple-600' : 'text-foreground'}`}>₹{a.toLocaleString('en-IN')}</Text>
            </Pressable>
          ))}
        </Animated.View>
        ...
        {otpSent && (
          <Animated.View entering={reducedMotion ? undefined : FadeInDown.duration(300)} className="mb-5">
            <Text className="mb-1.5 text-sm font-bold text-foreground">Enter OTP</Text>
            <TextInput placeholder="Transfer OTP" placeholderTextColor="#9ca3af" value={otp} onChangeText={setOtp} keyboardType="numeric" maxLength={6} className="rounded-xl border border-border bg-card px-4 py-3.5 text-center text-lg font-bold tracking-widest text-foreground" />
          </Animated.View>
        )}
        ...
        <Animated.View entering={reducedMotion ? undefined : FadeInDown.delay(320).duration(300)}>
          {!otpSent ? (
            <Pressable onPress={() => { if (!amount || parseFloat(amount) <= 0) { toast.error('Enter amount'); return; } setConfirmVisible(true); }}>
            ...
            <Pressable onPress={handleTransfer} disabled={loading}>
              <LinearGradient colors={['#8b5cf6', '#6366f1']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} className="items-center rounded-2xl py-4">
                {loading ? <ActivityIndicator color="white" /> : <Text className="text-base font-bold text-white">Transfer ₹{parseFloat(amount || '0').toLocaleString('en-IN')}</Text>}
              </LinearGradient>
            </Pressable>
          )}
        </Animated.View>
