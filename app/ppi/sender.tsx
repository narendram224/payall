  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const res: any = await Axios.post('v1/ppi/verification', fd);
        setSenderInfo(res);
      } catch (e: any) {
          ...
          <Animated.View entering={reducedMotion ? undefined : FadeInDown.duration(300)}>
            <View className="mb-5 rounded-2xl border border-border bg-card p-5">
              ...
              <View className="flex-row items-center gap-2">
                ...
                <Text className="text-sm font-semibold text-emerald-600">PPI Account Active</Text>
              </View>
              {senderInfo?.limit && (
                <View className="mt-3 rounded-xl bg-muted p-3">
                  <Text className="text-xs text-muted-foreground">Transfer Limit</Text>
                  <Text className="text-base font-bold text-foreground">
                    ₹{parseFloat(senderInfo.limit).toLocaleString('en-IN')}
                  </Text>
                </View>
              )}
            </View>
            ...
            <Pressable
              onPress={() =>
                router.push({ pathname: '/ppi/beneficiaries', params: { mobile_number } })
              }>
              <LinearGradient
                colors={['#8b5cf6', '#6366f1']}
                start={{ x: 0, y: 0 }}
