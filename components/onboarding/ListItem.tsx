import { View, useWindowDimensions, ImageURISource, StyleSheet } from 'react-native';
import React from 'react';
import Animated, { Extrapolate, interpolate, useAnimatedStyle } from 'react-native-reanimated';
import { SharedValue } from 'react-native-reanimated/lib/typescript/commonTypes';

type Props = {
  item: { text: string; image: ImageURISource };
  index: number;
  x: SharedValue<number>;
  currentIndex?: SharedValue<number>;
};

const ListItem = ({ item, index, x, currentIndex }: Props) => {
  const { width: SCREEN_WIDTH } = useWindowDimensions();

  const rnImageStyle = useAnimatedStyle(() => {
    // Use currentIndex if available, otherwise fall back to scroll position
    const isActive = currentIndex
      ? currentIndex.value === index
      : Math.abs(x.value - index * SCREEN_WIDTH) < SCREEN_WIDTH / 2;

    console.log(
      `ListItem ${index}: isActive=${isActive}, currentIndex=${currentIndex?.value}, x=${x.value}`
    );

    const translateY = interpolate(
      x.value,
      [(index - 1) * SCREEN_WIDTH, index * SCREEN_WIDTH, (index + 1) * SCREEN_WIDTH],
      [100, 0, 100],
      Extrapolate.CLAMP
    );
    const opacity = interpolate(
      x.value,
      [(index - 1) * SCREEN_WIDTH, index * SCREEN_WIDTH, (index + 1) * SCREEN_WIDTH],
      [0, 1, 0],
      Extrapolate.CLAMP
    );

    return {
      opacity: isActive ? opacity : 0.3,
      width: SCREEN_WIDTH * 0.7,
      height: SCREEN_WIDTH * 0.7,
      transform: [{ translateY }],
    };
  }, [index, x, currentIndex]);

  const rnTextStyle = useAnimatedStyle(() => {
    // Use currentIndex if available, otherwise fall back to scroll position
    const isActive = currentIndex
      ? currentIndex.value === index
      : Math.abs(x.value - index * SCREEN_WIDTH) < SCREEN_WIDTH / 2;

    console.log(
      `ListItem ${index} text: isActive=${isActive}, currentIndex=${currentIndex?.value}, x=${x.value}`
    );

    const translateY = interpolate(
      x.value,
      [(index - 1) * SCREEN_WIDTH, index * SCREEN_WIDTH, (index + 1) * SCREEN_WIDTH],
      [100, 0, 100],
      Extrapolate.CLAMP
    );
    const opacity = interpolate(
      x.value,
      [(index - 1) * SCREEN_WIDTH, index * SCREEN_WIDTH, (index + 1) * SCREEN_WIDTH],
      [0, 1, 0],
      Extrapolate.CLAMP
    );

    return {
      opacity: isActive ? opacity : 0.3,
      transform: [{ translateY }],
    };
  }, [index, x, currentIndex]);

  return (
    <View style={[styles.itemContainer, { width: SCREEN_WIDTH }]}>
      <Animated.Image source={item.image} style={rnImageStyle} resizeMode="contain" />
      <Animated.Text style={[styles.textItem, rnTextStyle]}>{item.text}</Animated.Text>
    </View>
  );
};

export default React.memo(ListItem);

const styles = StyleSheet.create({
  itemContainer: {
    flex: 1,
    alignItems: 'center',
    justifyContent: 'space-around',
  },
  textItem: {
    fontWeight: '600',
    lineHeight: 41,
    fontSize: 34,
    textAlign: 'center',
    paddingHorizontal: 20,
  },
});
